...
class LLM {
    ...
    private:
        ...
        std::vector<std::pair<fp_t, id_t>> get_top_predictions(
            const std::vector<fp_t>& logits,
            uint64_t batch_size);

        std::vector<id_t> sample_tokens(
            const std::vector<std::pair<fp_t, id_t>>& probabilities,
            uint64_t batch_size);
